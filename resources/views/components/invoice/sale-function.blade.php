...
<script>
    ...
    async function getClientsOnSale(){
        showLoader();
        let response = await axios('/customer-list');
        hideLoader();
    ...
    function add(){
        ...
        if(PName.length == 0){
            ...
            errorToast("Product quantity is requires");
        }else{
            let item = {product_name:PName, product_id:PId, sale_price:PTotalPrice, qty:PQty};
    ...
    async function CreateInvoice(){
        ...
        if(CId.length == 0){
            ...
            if(response.status === 200 && response.data['status'] === 'success'){
                successToast("Invoice has been created successfully");
                setTimeout(function (){
